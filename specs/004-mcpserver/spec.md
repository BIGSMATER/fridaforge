...
- **FR-004**: 系统 MUST 暴露"枚举设备"能力：返回已连接调试设备的列表信息（设备 ID、名称、连接类型）。初期可使用模拟数据。
- **FR-005**: 系统 MUST 暴露"枚举进程"能力：接受设备标识，返回该设备上运行进程的列表信息（进程 ID、进程名）。初期可使用模拟数据。
- **FR-006**: 每种能力 MUST 附带结构化输入参数定义和功能描述，使 AI 助手无需示例即可理解如何使用。
- **FR-007**: 系统 MUST 记录每次能力调用的信息（时间、能力名、参数摘要），用于审计和问题排查。
...
- **FR-012**: FridaForge 命令行 MUST 提供启动 MCP 服务的入口，使 AI 工具能将其作为子进程启动。
...
- MCP 是 AI 工具集成的事实标准协议，目标 AI 助手支持通过本地服务方式连接 MCP 服务。
