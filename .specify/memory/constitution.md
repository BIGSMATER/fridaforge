...
- 每个 Milestone 同步维护一个 HTML 版本教学文档至 `docs/learn/M[x]-*.html`，与 Markdown 版本**同等地位**（非实验性）。HTML 版本利用 `<details>/<summary>` 折叠区域、`<aside>` 标注框、`<dl>` 定义列表等语义元素增强可读性，同时提供 `@media print` 打印样式。两个版本内容须保持同步。
