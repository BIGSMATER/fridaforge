...
### 1.5 测试基础 — 为什么每个文件都配一个 `_test.go`

**测试是什么**：用代码来验证另一段代码是否正确。不是"编译通过 = 正确"——编译只能查语法，不能查逻辑。

**为什么需要测试**：你改了 A 函数，可能不小心破坏了 B 函数。没有测试 = 每次改代码都要手动重新验证所有功能 = 不可能做到。有测试 = 跑 `go test`，0.5 秒知道有没有破坏旧功能。

**Go 的测试三条规则**：

1. **文件名 `*_test.go`**：`validator.go` 配 `validator_test.go`。`go build` 会跳过 `_test.go` 文件——测试代码不进二进制。
2. **函数名 `Test` 开头**：`func TestValidate(t *testing.T)`。`go test` 自动找到所有 `Test` 开头的函数运行。
3. **参数 `*testing.T`**：这是测试的"遥控器"——`t.Error("失败")` 记录失败继续跑，`t.Fatal("致命")` 记录失败立即停，`t.Run("子测试", ...)` 创建独立子测试。

**在本项目测了什么（5 类）**：

| 测试文件 | 测什么 | 为什么 |
|---------|--------|--------|
| `types_test.go` | 常量值是否正确 | 常量拼错编译不报错，只有测试能抓 |
| `errors_test.go` | `FieldError.Error()` 输出格式 | 如果格式变了，CLI 输出也乱 |
| `validator_test.go` | 8 种规格各自的校验结果 | 如果你不小心改了逻辑，测试立刻失败 |
| `loader_test.go` | 读 YAML 文件 → 解析结构体 | 文件 I/O + YAML 解析两步都要对 |
| `manager_test.go` | 桩实现返回的数据正确 | M2 替换实现时，桩是参考基准 |

**TDD 的节奏（本项目全程使用）**：

```
1. 写桩 → 2. 写测试 → 3. 跑测试(FAIL) → 4. 写实现 → 5. 跑测试(PASS) → 6. 提交
```

这个循环确保：每一个实现完成的函数，都有测试在盯着它。

**覆盖率**：`go test -cover` 显示测试执行到了生产代码百分之多少的语句。100% = 每行代码至少被某个测试执行过。不是说你测全了，只是说没有"写了但从来没跑过的死代码"。
...
**模块路径解析 —— import 长得像 URL 但不用联网**：

初学 Go 最常见的困惑：写了 `import "github.com/bigsmater/fridaforge/pkg/config"`，代码还没推到 GitHub 上——会不会编译失败？

**不会。Go 编译器不访问 GitHub 来解析自己的包。**

判断逻辑很简单：

```go
// go.mod 第一行：
module github.com/bigsmater/fridaforge   // ← 模块根

// import 路径判断：
import "github.com/bigsmater/fridaforge/pkg/config"  
//      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ 匹配模块根 → 本地找！
//      go.mod 所在目录 + pkg/config =  /home/.../fridaforge/pkg/config/*.go

import "github.com/spf13/cobra"
//      ^^^^^^^^^^^^^^^^^^^^ 不匹配模块根 → 外部依赖 → 通过 GOPROXY / GitHub 下载
```

**只要 `go.mod` 在项目根目录，项目内所有子包的 import 都从本地解析。** GitHub 推没推上去完全不相关。

为什么模块名要用 `github.com/bigsmater/fridaforge`？因为 Go 模块系统的设计：**模块名 = 下载地址**。用这个格式，将来别人 import 你的包时 Go 知道去哪里下载。用 `module banana` 也完全合法，只是没法公开分享。

**为什么计划了 viper 但最终没用**

milestones.md 和最初 plan.md 都计划 M1 使用 viper 做配置绑定。实际代码中 `LoadSpec` 直接用 `os.ReadFile` + `yaml.Unmarshal`，没有 import viper。

原因：viper 的核心价值在"配置合并"——从多个来源（文件、环境变量、命令行参数）读取配置并按优先级合并。M1 的 `spec validate` 只做"给定一个文件路径 → 校验它"，不需要多来源合并。viper 在这里是杀鸡用牛刀——`os.ReadFile` 就够了。

M2+ 需要使用全局配置文件（`~/.fridaforge/config.yaml`）或环境变量覆盖时，viper 会被引入。届时教学文档会补充 viper 的使用方式。
...
### 1.20 `var _ Interface = (*Type)(nil)` — 编译时接口检查

```go
var _ DeviceLister = (*StubDeviceLister)(nil)
```

这行代码做了什么：把一个 `nil`（但类型是 `*StubDeviceLister`）赋值给 `DeviceLister` 接口变量，赋值结果丢弃给 `_`（空白标识符）。

这行代码的**唯一目的**是让编译器在赋值这行做接口实现检查：**「`*StubDeviceLister` 有没有 `DeviceLister` 要求的全部方法？」** 没有 → **编译报错**。有 → 编译通过，不产生任何运行时代码。

为什么需要它？因为 Go 没有 `implements` 关键字。如果不写这行检查，只有当有人真的在代码里写 `var lister DeviceLister = &StubDeviceLister{}` 时才会检查——而那段代码可能跟你不在同一个包、甚至还没写。

Docker、Kubernetes、etcd 等 Go 项目都用这个模式。放在接口定义旁边，谁签了合同就写一行。

### 1.21 `context.Context` — Go 的请求生命周期

```go
ListDevices(ctx context.Context) ([]Device, error)
```

`context.Context` 是 Go 标准库的类型，三个核心功能：

| 功能 | 用途 | 示例 |
|------|------|------|
| **超时控制** | "5 秒没返回就放弃" | `ctx, cancel := context.WithTimeout(ctx, 5*time.Second)` |
| **取消传播** | "用户按了 Ctrl+C，所有操作停" | 父 ctx 取消 → 所有子 ctx 自动取消 |
| **值传递** | 请求级别的数据（trace ID、user 等） | `ctx = context.WithValue(ctx, key, val)` |

M1 中传入的是 `context.Background()`——一个空的、永不取消、永不超时、无任何值的 context。**接口里放它是为 M2 做准备**：真实 Frida 查询可能很慢，需要超时控制。

### 1.22 `text/tabwriter` — Go 内置表格输出
...
`tabwriter` 自动对齐以 `\t`（Tab）分隔的列。对比手工对齐：

```go
// 不用 tabwriter——你得自己算宽度：
fmt.Printf("%-16s %-22s %s\n", d.ID, d.Name, d.ConnectType)
//          ^^^^^^^^ 每次加新设备都要改列宽！
```

用 tabwriter 的话，它内部自动计算每列最大宽度再输出——**永远对齐，不管内容多长**。

`NewWriter` 参数 (w, minWidth, tabWidth, padding, padChar, flags)：
- 前两个 0 = 不设最小宽度
- 2 = 列间 padding 2 个空格
- `Flush()` 是触发输出的命令，tabwriter 缓冲数据到 Flush 再写
...
**突破一：宪法不是摆设——analyze 让它"活过来"**

宪法 2.2 的接口命名规则（单方法接口以 `-er` 结尾）在代码里被违反了一次——`DeviceManager` 是单方法接口但以 `Manager` 结尾。`/speckit.analyze` 在交叉验证 spec/plan/tasks 时抓出了这个问题，重命名为 `DeviceLister`。

教训：宪法条文如果不和实际代码交叉验证，就是一篇没人看的 markdown。analyze 阶段的价值在于让宪法"活过来"——机器对照检查，人做决策。

**突破二：三个文档一定会漂移**

spec.md、plan.md、tasks.md 如果分别写，一定会互相矛盾。不是写的人不认真——是人脑无法同时追踪三个文档的每一条声明。当文件总量达到 233 行（tasks.md）+ 94 行（plan.md）+ 125 行（spec.md）= 452 行时，任何一个字的偏差都可能被漏掉。analyze 阶段在这次 M1 中发现了 8 个不一致，其中 1 个 CRITICAL、3 个 MEDIUM。

这印证了 Spec Coding 的核心主张：**把验证交给机器，把决策留给人。**

**突破三：教学文档是检验理解的唯一标准**

写 `docs/learn/M1-go-cli-config.md` 的过程暴露了"以为自己懂了"和"真正能讲清楚"之间的差距。以接口概念为例：
- 第一版：用"鸭子类型"比喻——学员表示不理解
- 第二版：用"隐式满足"术语——学员仍然困惑
- 第三版：用"合同隐喻" + FridaForge M1/M2 替换场景——学员懂了

教训：最好的学习方式是尝试教会别人。如果不能用三种不同的比喻讲清楚一个概念，说明自己还没真正懂。教学文档就是这个"别人"。

**突破四：教学和编码不应该割裂**

M1 开始时试图"全部讲完再全部写"——学员看完整套 Go 语法后要求"开始写"，写了两个 Task 后接口概念暴露出了真正的盲区。如果在教学阶段就讲接口，学员会觉得"哦，Java 里类似"——没有实际代码的对照，理解停留在表面。

M1 证明"即讲即写"模式更有效：每个 Phase 写代码前讲新概念，代码写完补充学习文档，学员的困惑在代码层面暴露并在当场解决。

### 3.4 M1 工作流反思（供 M2 参考）

| 哪些做对了 | 哪些需要改进 |
|-----------|-------------|
| SpecKit 规划阶段严格顺序执行 | 教学文档初始版过早——核心概念讲完应立刻进入实现 |
| 每个 Phase = 讲解 → 编码 → 补充文档 | 不要自顾自推完所有 Task——每个 Phase 等确认 |
| analyze 阶段执行了两次（实现前 + 实现后） | viper 这种"计划了但没用"的要在教学文档里解释 |
| 同文件 Task 合并 Commit | milestones.md 的目标产出要在实现后更新 |
| 学习文档随代码逐步丰富 | 独立迷你示例和项目代码示例要形成对照 |
...
| go.mod / go.sum / `// indirect` / 模块路径本地解析 | T002 | M1 |
...
| `var _ Interface = (*Type)(nil)` 编译时检查 | T019 | M1 |
| `context.Context` 请求生命周期 | T019 | M1 |
| `text/tabwriter` 表格对齐输出 | T020 | M1 |

---

> **宪法 6.1 确认**：本学习笔记同等覆盖了 Go 语法特性（第一章）、Android/Frida 逆向原理（第二章）、SpecKit 工作流与 AI 编程认知突破（第三章）。M1 完成。

---

## 附录 A：独立可运行迷你代码示例

> 以下示例脱离 FridaForge 项目上下文，可单独保存为 `.go` 文件运行。
> 每个示例对应正文中的教学章节，供概念学习和快速验证使用。
...
func main() {
    cart := []string{"苹果", "香蕉", "橘子"}
    prices := map[string]int{"苹果": 5, "香蕉": 3, "橘子": 4}

    total := 0
    for _, item := range cart {
        total += prices[item]
    }
    fmt.Printf("购物车总价: %d 元\n", total)
}
```

### A.5 cobra 迷你 CLI（对应 1.6）
...
func main() {
    var n Notifier
    n = Email{}
    n.Notify("你好")   // 输出: 邮件: 你好
    n = SMS{}
    n.Notify("你好")   // 输出: 短信: 你好
}
```
项目对应：`DeviceLister` 接口 → `StubDeviceLister`（M1 桩）和 `RealDeviceLister`（M2 真实 Frida）。
